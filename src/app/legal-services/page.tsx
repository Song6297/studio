...
import { CalendarCheck, Users, Video, Phone } from 'lucide-react';
import { useLanguage } from '@/context/language-context';
import { cn } from '@/lib/utils';
import { format } from 'date-fns';

function ConsultationBookingForm() {
    const { t } = useLanguage();
    ...
    function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        toast({
            ...
            description: `${t('legalServices.toast.successDescription')} ${format(selectedDate, 'PPP')} at ${selectedTime}`,
        });
        form.reset();
    ...
    return (
        <Card className="bg-card/80 backdrop-blur-sm">
            <CardHeader>
                <CardTitle className="font-headline text-2xl">{t('legalServices.booking.title')}</CardTitle>
                <CardDescription className="text-muted-foreground">{t('legalServices.booking.description')}</CardDescription>
            </CardHeader>
            <CardContent>
                <Form {...form}>
